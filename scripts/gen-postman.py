...
Generates a Postman collection by dynamically parsing:
  src/Cqrs.OrderService/graphql/*.graphqls              -> GraphQL queries + mutations
  src/Cqrs.OrderService/Presentation/**/*Controller.cs  -> REST endpoints

Adding a new ASP.NET Core controller action or GraphQL operation is picked up automatically.
"""
...
def _parse_graphql():
    files = _glob("src/Cqrs.OrderService/graphql/**/*.graphqls")
    if not files:
        return [], [], {}
...
def _field_example(name, base_type, default=None):
    examples = {
        "categoryId": "",
    ...
    return _SCALARS.get(base_type, None)


def _example(base_type, named_types, default=None, name=None):
    ...
    if base_type in _SCALARS:
        return _field_example(name, base_type) if name else _SCALARS[base_type]
    # Input type — build a typed nested object
    if base_type in named_types:
        return {f["name"]: _field_example(f["name"], f["base_type"]) for f in named_types[base_type]}
    return None
...
def _gql_item(op, is_mutation, named_types):
    ...
    variables = {p["name"]: _example(p["base_type"], named_types, p["default"], p["name"]) for p in params}
...
_HTTP_MAP = {"Get": "GET", "Post": "POST", "Put": "PUT",
             "Delete": "DELETE", "Patch": "PATCH"}


def _route_join(base, child):
    parts = [p.strip("/") for p in (base, child) if p is not None and p.strip("/")]
    return "/" + "/".join(parts)


def _attr_path(attrs, name):
    patterns = [
        rf'\[{name}\s*\(\s*"([^"]*)"\s*\)\s*\]',
        rf'\[{name}\s*\(\s*template\s*:\s*"([^"]*)"\s*\)\s*\]',
    ]
...
def _query_params(params_text):
    query_params = []
    for param in _split_params(params_text):
        if "[FromQuery]" not in param:
            continue
        cleaned = re.sub(r"\[[^\]]+\]", " ", param)
        cleaned = re.sub(r"\s+", " ", cleaned).strip()
        pm = re.match(r"[\w<>,.?]+\s+(\w+)(?:\s*=\s*([^,]+))?", cleaned)
        if not pm:
            continue
        default = (pm.group(2) or "").strip()
        default = "" if default in ("null", "default") else default.strip('"')
        query_params.append({"name": pm.group(1), "default": default})
    return query_params


def _parse_controllers():
    controllers = []
    for path in _glob("src/Cqrs.OrderService/Presentation/**/*Controller.cs"):
        content = open(path).read()
        if "[ApiController]" not in content:
            continue
        class_m = re.search(r"public\s+(?:sealed\s+)?class\s+(\w+)", content)
        if not class_m:
            continue
        ...
        class_attrs = content[:class_m.start()]
        base = _attr_path(class_attrs, "Route")

        endpoints = []
        ann_pat = re.compile(
            r'(?P<attrs>(?:\s*\[[^\]]+\]\s*)+)'
            r'\s*public\s+(?:async\s+)?[^{(]+?\s+(?P<name>\w+)\s*\(',
            re.DOTALL
        )
        for am in ann_pat.finditer(content):
            attrs = am.group("attrs")
            http_m = re.search(r"\[Http(Get|Post|Put|Delete|Patch)(?:\s*\(\s*\"([^\"]*)\"\s*\))?\s*\]", attrs)
            if not http_m:
                continue
            http_method = _HTTP_MAP[http_m.group(1)]
            full_path = _route_join(base, http_m.group(2) or "")

            method_name = am.group("name")
            display = re.sub(r"([A-Z])", r" \1", method_name).strip().title()
            ...
            has_body = has_body or "[FromBody]" in params_text
            query_params = _query_params(params_text)
...
def main():
    ...
    collection = {
        "info": {
            ...
            "description": "Production-grade CQRS Order Service - ASP.NET Core / .NET 10",
